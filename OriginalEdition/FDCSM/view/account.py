...
def login(request):
    ...
    if form.is_valid():
        admin_object = models.FDC_ADM_INFO.objects.filter(ADM_NBR=form.cleaned_data['username'],
                                                          ADM_PWD=form.cleaned_data['password']).first()
        if not admin_object:
            form.add_error('password', '用户名或密码错误')
            return render(request, 'login.html', {'form': form})

        request.session['info'] = {'id': admin_object.ADM_NBR, 'name': admin_object.ADM_NAM}
...
def stu_login(request):
    ...
    form = LoginForm(request.POST)
    if form.is_valid():
        stu_object = models.FDC_STU_INFO.objects.filter(STU_NBR=form.cleaned_data['username'],
                                                          STU_PWD=form.cleaned_data['password']).first()
        if not stu_object:
            form.add_error('password', '用户名或密码错误')
            return render(request, 'stu_login.html', {'form': form})
...
def pfs_login(request):
    ...
    form = LoginForm(request.POST)
    if form.is_valid():
        pfs_object = models.FDC_PFS_INFO.objects.filter(PFS_NBR=form.cleaned_data['username'],
                                                          PFS_PWD=form.cleaned_data['password']).first()
        if not pfs_object:
            form.add_error('password', '用户名或密码错误')
            return render(request, 'stu_login.html', {'form': form})
