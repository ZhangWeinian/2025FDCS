...
def student_excel_add(request):
    ...
    for row in ws.iter_rows(min_row=2):
        ...
        data = {'STU_NBR': row[0].value, 'STU_NAM': row[1].value,
                'STU_PWD': row[2].value, 'STU_PRO': stu_pro_choices[row[3].value]}
        if not row_obj:
...
def student_detail(request):
    uid = request.GET.get("uid")
    row_dict = models.FDC_STU_INFO.objects.filter(STU_NBR=uid).values('STU_NBR', 'STU_NAM', 'STU_PWD', 'STU_TEL_NBR',
                                                                      'STU_PRO').first()
    if not row_dict:
...
def student_chose(request):
    ...
    if stu_info.STU_PRO == 1:  # 数学专业
        querySet = models.FDC_PFS_INFO.objects.annotate(
            select_count=Count('fdc_pfs_sel', Q(fdc_pfs_sel__STU_PRO=1))).exclude(
            Q(PFS_NUM_MATH=0) | Q(select_count__gte=F('PFS_NUM_MATH')))
    elif stu_info.STU_PRO == 2:  # 系统科学专业
        querySet = models.FDC_PFS_INFO.objects.annotate(
            select_count=Count('fdc_pfs_sel', Q(fdc_pfs_sel__STU_PRO=2))).exclude(
            Q(PFS_NUM_SCI=0) | Q(select_count__gte=F('PFS_NUM_SCI')))
    elif stu_info.STU_PRO == 0:  # 物理专业
        querySet = models.FDC_PFS_INFO.objects.annotate(
            select_count=Count('fdc_pfs_sel', Q(fdc_pfs_sel__STU_PRO=0))).exclude(
...
def student_select(request):
    ...
    if stu_info.first().STU_TYP == 1:
        return JsonResponse({"status": False, "error": "你已经被导师录用，不用再选择导师"})
    if stu_sel_obj:
        if not stu_sel_obj.first().STU_ONE_SEL_id:
            stu_sel_obj.update(STU_ONE_SEL_id=teacher_id, CHG_TIM=timezone.now())
        elif not stu_sel_obj.first().STU_TOW_SEL_id:
            stu_sel_obj.update(STU_TOW_SEL_id=teacher_id, CHG_TIM=timezone.now())
        elif not stu_sel_obj.first().STU_THR_SEL_id:
            stu_sel_obj.update(STU_THR_SEL_id=teacher_id, CHG_TIM=timezone.now())
        else:
            return JsonResponse({"status": False, "error": "不能再选导师"})
    else:
        models.FDC_STU_INTRO.objects.create(STU_NBR_id=info_id, STU_ONE_SEL_id=teacher_id)
    return JsonResponse({"status": True})
...
def student_delete_sel(request):
    ...
    obj.update(STU_ONE_SEL=None, STU_TOW_SEL=None, STU_THR_SEL=None, CHG_TIM=timezone.now())
