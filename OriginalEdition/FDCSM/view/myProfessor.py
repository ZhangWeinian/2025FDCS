...
def professor_detail(request):
    ...
    row_dict = models.FDC_PFS_INFO.objects.filter(PFS_NBR=uid).values('PFS_NBR', 'PFS_NAM', 'PFS_PWD', 'PFS_TEL_NBR',
                                                                      'PFS_INTRO', 'PFS_NUM_MATH',
                                                                      'PFS_NUM_PSC', 'PFS_NUM_SCI').first()
    if not row_dict:
...
def professor_chose(request):
    ...
    form = ProfessorModelForm()
    if status.STAT_ID == 1:
        querySet = models.FDC_STU_INTRO.objects.filter(STU_ONE_SEL=uid, STU_NBR__STU_TYP=0)
    elif status.STAT_ID == 2:
        querySet = models.FDC_STU_INTRO.objects.filter(STU_TOW_SEL=uid, STU_NBR__STU_TYP=0)
    elif status.STAT_ID == 3:
        querySet = models.FDC_STU_INTRO.objects.filter(STU_THR_SEL=uid, STU_NBR__STU_TYP=0)
    else:
...
def professor_select(request):
    ...
    count_select = models.FDC_PFS_INFO.objects.annotate(
            select_count=Count('fdc_pfs_sel', Q(fdc_pfs_sel__STU_PRO=int(stu_pro)))).filter(
            Q(select_count__lt=F(pro_choice[stu_pro])) & Q(PFS_NBR=pfs_id)).count()
    if not count_select:
...
def professor_select_delect(request):
    ...
    pfs_id = request.session.get("info")['id']
    status = models.FDC_STAT_INFO.objects.values('STAT_ID').all().first()
    select_obj = models.FDC_PFS_SEL.objects.filter(PFS_NBR=pfs_id, PFS_STU_NBR=stu_id).first()
