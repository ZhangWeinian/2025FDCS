...
def admin_detail(request):
    uid = request.GET.get("uid")
    row_dict = models.FDC_ADM_INFO.objects.filter(ADM_NBR=uid).values('ADM_NBR', 'ADM_NAM', 'ADM_PWD').first()
    if not row_dict:
...
def status(request):
    ...
    status = {0: "开启学生选择志愿", 1: "开启导师第一志愿", 2: "开启导师第二志愿", 3: "开启导师第三志愿"}
...
def status_s(request):
    import django.utils.timezone as timezone
    status_choise = {0: "开启学生选择志愿", 1: "开启导师第一志愿", 2: "开启导师第二志愿", 3: "开启导师第三志愿"}
...
def deleteAdmin(request):
    ...
    models.FDC_PFS_SEL.objects.filter(PFS_NBR=pfs_id, PFS_STU_NBR=stu_id).first().delete()
