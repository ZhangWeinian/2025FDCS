...
class FDC_STU_INFO(models.Model):
    ...
    STU_TEL_NBR = models.CharField(verbose_name='电话号码', max_length=20, blank=True, null=True)
    stu_pro_choices = (
        (0, '物理专业'),
        (1, '数学专业'),
        (2, '系统科学专业')
    ...
    STU_TYP = models.IntegerField(verbose_name='状态', default=0, choices=stu_typ_choices)
...
class FDC_STU_INTRO(models.Model):
    STU_NBR = models.OneToOneField(FDC_STU_INFO, on_delete=models.CASCADE, primary_key=True)
    STU_ONE_SEL = models.ForeignKey(to='FDC_PFS_INFO', to_field='PFS_NBR', on_delete=models.SET_NULL, null=True, related_name='STU_ONE_SEL')
    STU_TOW_SEL = models.ForeignKey(to='FDC_PFS_INFO', to_field='PFS_NBR', on_delete=models.SET_NULL, null=True, related_name='STU_TOW_SEL')
    STU_THR_SEL = models.ForeignKey(to='FDC_PFS_INFO', to_field='PFS_NBR', on_delete=models.SET_NULL, null=True, related_name='STU_THR_SEL')
    STU_GRADE = models.FileField(verbose_name='成绩', max_length=100, upload_to='grade/', null=True, blank=True)
    STU_VIT = models.FileField(verbose_name='简历', max_length=100, upload_to='intro/', null=True, blank=True)
    CAT_TIM = models.DateTimeField(verbose_name='用户创建时间', default=timezone.now)
    CHG_TIM = models.DateTimeField(verbose_name='用户更新时间', auto_now=True)
    class Meta:
...
class FDC_PFS_INFO(models.Model):
    ...
    PFS_TEL_NBR = models.CharField(verbose_name='电话号码', max_length=20, blank=True, null=True)
...
class FDC_PFS_SEL(models.Model):
    ...
    PFS_NBR = models.ForeignKey(to='FDC_PFS_INFO', to_field='PFS_NBR', on_delete=models.CASCADE)
    PFS_STU_NBR = models.ForeignKey(to='FDC_STU_INFO', to_field='STU_NBR', on_delete=models.SET_NULL, null=True, related_name='PFS_STU_NBR')
    stu_pro_choices = (
        (0, '物理专业'),
        (1, '数学专业'),
        (2, '系统科学专业')
...
class FDC_STAT_INFO(models.Model):
    ...
    STAT_ID = models.IntegerField(verbose_name='当前状态', primary_key=True, choices=stat_id_choises)
    CAT_TIM = models.DateTimeField(verbose_name='用户创建时间', default=timezone.now)
    CHG_TIM = models.DateTimeField(verbose_name='用户更新时间', auto_now=True)
    class Meta:
