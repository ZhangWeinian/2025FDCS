...
class AuthMiddleware(MiddlewareMixin):
    def process_request(self, request):
        ...
        if request.path_info in admin_views:
            admin_object = models.FDC_ADM_INFO.objects.filter(ADM_NBR=info_dict['id']).first()
            if admin_object:
                return
        ...
        if request.path_info in pfs_views:
            ...
            pfs_object = models.FDC_PFS_INFO.objects.filter(PFS_NBR=info_dict['id']).first()
            if pfs_object:
                return
            return redirect('/pfs_login/')
        # 学生访问页面
        ...
        if request.path_info in stu_views:
            ...
            stu_object = models.FDC_STU_INFO.objects.filter(STU_NBR=info_dict['id']).first()
            if stu_object:
                return
            return redirect('/stu_login/')